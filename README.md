
## Descripción
**Paint_JAVR** es una innovadora aplicación de pintura que permite a los usuarios crear y editar dibujos de manera intuitiva. Este proyecto utiliza técnicas de computación tolerante para asegurar un rendimiento óptimo y una experiencia de usuario fluida.

## Características
- **Herramientas de Dibujo:** Pincel, borrador, formas y más.
<img width="996" height="680" alt="image" src="https://github.com/user-attachments/assets/b3d8804a-05cf-4cb4-a69b-75ae98e6b5ae" />

- **Paleta de Colores:** Selección de múltiples colores para personalizar tus creaciones.
<img width="1004" height="415" alt="image" src="https://github.com/user-attachments/assets/5811893a-e0f3-4098-9cbd-ec70888603a4" />

- **Funcionalidad de Deshacer/Rehacer:** Mantén el control total sobre tus ediciones.
<img width="998" height="422" alt="image" src="https://github.com/user-attachments/assets/b119bc52-27ac-4439-99fb-fe3a1dcf559e" />

- **Guardar:** Guarda tus obras maestras y vuelve a editarlas más tarde.
<img width="1001" height="423" alt="image" src="https://github.com/user-attachments/assets/549f8f31-7b2d-40bb-8e40-ce3e480cd67d" />


## Requisitos
- **Python 3.x**
- **Bibliotecas necesarias:** Asegúrate de tener instaladas las siguientes bibliotecas:
  - `tkinter`
  - `Pillow`

## Instalación
1. **Clona el repositorio:**
   ```bash
   git clone <URL del repositorio>
   ```
2. **Navega al directorio del proyecto:**
   ```bash
   cd Paint_JAVR
   ```
3. **Instala las dependencias:**
   ```bash
   pip install -r requirements.txt
   ```

## Uso
Ejecuta el archivo principal para iniciar la aplicación:
```bash
python main.py
```

## Contribuciones
Las contribuciones son bienvenidas. Si deseas contribuir, por favor abre un issue o un pull request para discutir tus cambios.

## Licencia
Este proyecto está bajo la Licencia MIT. Consulta el archivo LICENSE para más detalles.

## Contacto
Para más información, puedes contactar a:
- **Nombre:** Jordi Andony Vargas Ruiz
- **Email:** jordi_vr2000@hotmail.com
- **GitHub:** JordiVargasRuiz
